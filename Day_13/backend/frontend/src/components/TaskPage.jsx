...
const TaskPage = ()=>{
    ...
    return(
        <>
            <div>
                <h2>Tasks Assigned to Employee with ID {empId}</h2>

                <button onClick={() => navigate("/employee")}>⬅ Back</button>

                {isAdmin && (
                    <div>
                        <button onClick={()=>handleAddTask(empId)} className="btn btn-primary">+ Add Task</button>
                    </div>
                )}
                <br/>
                    ...
                    <tbody>
                        {tasks.map((task)=>(
                            <tr key={task.taskId}>
                                ...
                                {isAdmin && (
                                    <td>
                                        <button onClick={()=>handleEdit(task.taskId)} className="btn btn-outline-warning">edit</button>
                                        <button onClick={()=>handleDelete(task.taskId)} className="btn btn-outline-danger">delete</button>
                                    </td>
                                )}
                            </tr>
                        ))}
